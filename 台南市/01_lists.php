...
$tmpPath = dirname(__DIR__) . '/tmp/tainan';
...
for (; $currentPage <= $totalPages; $currentPage++) {
    $listUrl = 'http://asms.wsn.com.tw/TN/ieland/el_LoseList.aspx?Page=' . $currentPage;
                    ...
                    foreach ($fields AS $fieldKey => $field) {
                        switch ($fieldKey) {
                            ...
                            case 6:
                                ...
                                $record['url'] = 'http://asms.wsn.com.tw/TN/ieland/el_LoseDetail.aspx?ID=' . $record['id'] . '&fn=Photo&tn=SheltersReco';
                                break;
                            case 7:
                                $field = explode('../Upload/', $field);
                                $record['image_url'] = '';
                                if (isset($field[1])) {
                                    $record['image_url'] = 'http://asms.wsn.com.tw/TN/Upload/' . substr($field[1], 0, strpos($field[1], '"'));
                                }
                                break;
                            default:
                                $field = explode('：', trim(strip_tags($field)));
